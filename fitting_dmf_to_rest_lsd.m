...
%% Running optimizer: Optimizing G and alpha
% Bayes optimization options
sel_c = 1;
prev_opt = load([savefold,'bayesopt_dmf_checkpoint_G_alpha_',condnames{sel_c},'.mat']);

savefold = '/media/ruben/ssd240/Matlab/fastdmf-master/newSciRep/';
savename = [savefold,'bayesopt_dmf_checkpoint_G_alpha_',condnames{sel_c},'_v2.mat'];
...
T = 510;
[opt_fc_error,opt_fcd_ks,opt_pars,bayesopt_out] = fit_fc_fcd_dmf_only_slope(...
    T,ave_fc(:,:,sel_c),fcd(:,:,:,sel_c),G,fic_alpha,params,bo_opts);
...
%% Predict selected point
Xtable = table(2.4,1.5,'VariableNames',par_names);
[sel_objective,sel_sigma] = predictObjective(Bayesop68tResults,Xtable)
